...
namespace eva_april {
    ...
    void CamSubscriber::StartSubscribe() {
        is_start_sub = true;
    }

    void CamSubscriber::StopSubscribe() {
        is_start_sub = false;
    }

    void CamSubscriber::ParseData(bool init, bool init_lio, bool init_viw, bool stop_flag) {
        buff_mutex_.lock();
        while (!new_cam_data_.empty()){
            ...
            AprilTag_buf_push(item, init, init_lio, init_viw);
        }
        buff_mutex_.unlock();
    }
