                      ...
                      std::vector<cv::Point2f> &p2ds)
{
  ...
  else if (APRIL == pt)
  {
    ...
    const double tag_sz = 0.0352;
    const double tag_spacing_sz = 0.04576; // 0.0352 + 0.01056 
    ...
    if (detections.size() > 20)
    {
      ...
      cv::putText(img_raw, "X", xaxis, cv::FONT_HERSHEY_SIMPLEX, 0.5,
                  cv::Scalar(0, 0, 255));
      cv::line(img_raw, center, yaxis, cv::Scalar(0, 255, 0), 2);
      cv::putText(img_raw, "Y", yaxis, cv::FONT_HERSHEY_SIMPLEX, 0.5,
                  cv::Scalar(0, 255, 0));
      cv::line(img_raw, center, zaxis, cv::Scalar(255, 0, 0), 2);
      cv::putText(img_raw, "Z", zaxis, cv::FONT_HERSHEY_SIMPLEX, 0.5,
                  cv::Scalar(255, 0, 0));*/
                  ...
                  Eigen::Matrix4d &Twc, cv::Mat &img_raw,const camodocal::CameraPtr &cam)
{
  ...
  cv::putText(img_raw, "X", cv::Point2f(imgpts[1](0), imgpts[1](1)), cv::FONT_HERSHEY_SIMPLEX, 0.5,cv::Scalar(255, 0, 0));
  cv::putText(img_raw, "Y", cv::Point2f(imgpts[2](0), imgpts[2](1)), cv::FONT_HERSHEY_SIMPLEX, 0.5,cv::Scalar(0, 255, 0));
  cv::putText(img_raw, "Z", cv::Point2f(imgpts[3](0), imgpts[3](1)), cv::FONT_HERSHEY_SIMPLEX, 0.5,cv::Scalar(0, 0, 255));
