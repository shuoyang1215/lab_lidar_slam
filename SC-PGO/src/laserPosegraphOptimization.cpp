...
std::vector<pcl::PointCloud<PointType>::Ptr> keyframeLaserClouds_save; 
...
void process_pg()
{
    ...
    while (1)
    {
        ...
        while (!odometryBuf.empty() && !fullResBuf.empty())
        {
            ...
            mKF.lock();
            keyframeLaserClouds_save.push_back(thisKeyFrame); 
...
void save_odo()
{
    std::cout << "odo save_path = " << save_path << std::endl;
    mKF.lock();
    for (int node_idx = 0; node_idx < int(keyframePosesUpdated.size()); node_idx++)
    {
        const Pose6D &pose_est = keyframePosesUpdated.at(node_idx); // upodated poses
...
void save_map()
{
    ...
    pcl::PointCloud<PointType>::Ptr laserCloudMapLocalPGO(new pcl::PointCloud<PointType>());  // 局部地图

    mKF.lock();
    laserCloudMapPGO->clear();
    // std::cout << "int(keyframePosesUpdated.size()): " << int(keyframePosesUpdated.size()) << std::endl;
    // 保存局部地图前创建文件夹
    createDirectory(save_pcd_path + "pcds_ori/");
    for (int node_idx = 0; node_idx < int(keyframePosesUpdated.size()); node_idx++)
    {
        /*  - ---------------------------保存局部地图  --------------------------------*/
        std::stringstream idx;
        idx<<std::setw(6)<<std::setfill('0')<<node_idx;
        std::string res = idx.str();
        laserCloudMapLocalPGO -> clear();
        *laserCloudMapLocalPGO = *keyframeLaserClouds_save[node_idx];
        std::string local_map_dir(save_pcd_path + "pcds_ori/"+ res + ".pcd");
        downSizeFilterMapSave.setInputCloud(laserCloudMapLocalPGO); //  共用 downSizeFilterMapSave filter size = 0.1
        downSizeFilterMapSave.filter(*laserCloudMapLocalPGO);
        pcl::io::savePCDFile(local_map_dir, *laserCloudMapLocalPGO);
        /* -------------------------------------------------------------------------------- */
        
        *laserCloudMapPGO += *local2global(keyframeLaserClouds[node_idx], keyframePosesUpdated[node_idx]);
    }
    downSizeFilterMapSave.setInputCloud(laserCloudMapPGO);
    downSizeFilterMapSave.filter(*laserCloudMapPGO);
    // std::cout << "laserCloudMapPGO->points.size(): " << laserCloudMapPGO->points.size() << std::endl;
    pcl::io::savePCDFile(all_points_dir, *laserCloudMapPGO);
    std::cout << "Global map has been saved!!! "<< std::endl;
    mKF.unlock();
} // pointcloud_viz
...
void save_process(){
    ...
    if (s == "s"){
        ROS_WARN("start saving process!!!");  
        flag_exit = true;
        save_odo();
        save_map();
        process_interactive_slam_result();
        ROS_WARN("finish saving process!!!");  
        int ret = system("roslaunch map_conversion grid_2d_generation.launch");
        if (ret == -1) ROS_WARN("Failed to convert 2D grid map!!!");  
        else ROS_WARN("Succeeded to convert 2D grid map!!!");  
    }     
}
...
int main(int argc, char **argv)
{
    ...
    double mapSaveFilterSize;
    double sc_filter_size;

    nh.param<double>("mapviz_filter_size", mapVizFilterSize, 0.4);   // pose assignment every k frames
    nh.param<double>("mapsave_filter_size", mapSaveFilterSize, 0.4); // save map;
    nh.param<double>("sc_filter_size", sc_filter_size, 0.4); // save map;
    ...
    downSizeFilterICP.setLeafSize(sc_filter_size, sc_filter_size, sc_filter_size);
    ...
    std::thread save_all{save_process}; // visualization - path (high frequency)

    ros::spin();

    ROS_INFO("shutting down in main!!!");
