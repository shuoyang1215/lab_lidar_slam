...
void publish_frame_body(const ros::Publisher &pubLaserCloudFull_body)
{
    ...
    // std::cerr << "feats_undistort . size() " << feats_undistort->size() << std::endl;
...
int main(int argc, char **argv)
{
    ...
    nh.param<bool>("map_mode", p_pre->map_mode, false);           // 采样间隔，即每隔point_filter_num个点取1个点
    ...
    fout_lio.open(ros::package::getPath("sfast_lio") + "/log/lio_livox_slam_mapping.txt",ios::out);
    ...
    downSizeFilterGlobalMap.setLeafSize(0.05, 0.05, 0.05);
    ...
    while (ros::ok())
    {
        ...
        if (sync_packages(Measures)) //把一次的IMU和LIDAR数据打包到Measures
        {
            ...
            // std::cout << "feats_down_size: " << feats_down_size <<  "  avg mapping time(ms):  " << total_time / total_frame <<  "  cur mapping time(ms):  " << (t11 - t00) * 1000 << std::endl
            //           << std::endl;
            fout_lio << "feats_down_size: " << feats_down_size << "  avg mapping time(ms):  " << total_time / total_frame << "  cur mapping time(ms):  " << (t11 - t00) * 1000 << std::endl
                      << std::endl;
    ...
    if (pcl_wait_save->size() > 0 && pcd_save_en)
    {
        ...
        downSizeFilterGlobalMap.setInputCloud(pcl_wait_save);
        downSizeFilterGlobalMap.filter(*pcl_wait_save);
        pcd_writer.writeBinary(all_points_dir, *pcl_wait_save);
