...
void Preprocess::mid360_handler(const sensor_msgs::PointCloud2_<allocator<void>>::ConstPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  for(uint i=1; i<plsize; i++)
  {
    // 发布的并不是相对时间 而是以ns为单位的绝对时间 所以在此处的回调中进行处理    
    ...
    if (valid_num % point_filter_num == 0)
    {
      ...
      // 计算相对时间并统一单位ns->ms
      pl_full[i].curvature = (pl_orig.points[i].timestamp - first_point_time) / double(1000000);
     
      if(map_mode){  // 建图模式为了防止过多动态物体，移除后方的点
        if(pl_full[i].x < 0.0)
          continue;
      }
     
      double dist_ = pl_full[i].x * pl_full[i].x + pl_full[i].y * pl_full[i].y + pl_full[i].z * pl_full[i].z;
      if((dist_ > (blind * blind)) && dist_ < 100 * 100) // 排除过远的 “鬼影点”  , 只保留 180 的 FOV
      {
        pl_surf.push_back(pl_full[i]);
      }
    }
