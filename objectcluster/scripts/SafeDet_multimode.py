...
def lidar_callback(cloud):
    ...
    X_GRID_NUM = 27         # X方向栅格数，即(X_MAX-X_MIN)/GRID_SIZE+1，需结合范围调整 20220522 # 更改！
    Y_GRID_NUM = 16         # Y方向栅格数，即(Y_MAX-Y_MIN)/GRID_SIZE+1，需结合范围调整 20220522 # 更改！
    X_GRID_OFFSET = 13       # 将栅格地图索引转换为正值的偏移量，-X_MIN/GRID_SIZE，需结合范围调整 20220522
    ...
    # 栅格内达到此点数即为障碍物栅格
    plane_grid_point_thr = 3     # 20221025
    grass_grid_point_thr = 10     # 20221025

    # 平地时点云的 z 轴坐标范围
    stick_z0 = -0.61
    stick_z1 = -0.56        # 20221025
    stick_z2 = -0.1
    # 草地时点云的 z 轴坐标范围
    person_z1 = -0.38
    person_z2 = -0.1
    ...
    if (Output_OnePC):
        for i in range(1):
            Output_PC.append([100, 100, 100])


    
    if (Output_multiPC):
        for i in range(X_GRID_NUM * Y_GRID_NUM * 4):
            Output_PC.append([100, 100, 100])
    ...
    for i in range(points.shape[0]):
        ...
        if ((point[0] > X_MIN) and (point[0] < X_MAX) and (point[1] > Y_MIN) and (point[1] < Y_MAX)):
            if ((point[2] > stick_z1) and (point[2] < stick_z2)):
                ...
                if (point[2]>max_point[2]): max_point[2] = point[2] # 更改！
                if (point[2]<min_point[2]): min_point[2] = point[2] # 更改！
            elif ((point[2] > stick_z0) and (point[2] < stick_z1)):
                ground_count += 1


    print("point after filter:" + str(count) + " ground_points:" + str(ground_count))
    

    # 如果是草地,第二次过滤地面点 person, 并分配到格子中
    dis2 = (max_point[0]-min_point[0])*(max_point[0]-min_point[0]) + (max_point[1]-min_point[1])*(max_point[1]-min_point[1])    # 更改！
    print("length:" + str(math.sqrt(dis2)) + " height:" + str(max_point[2]-min_point[2]))
    if ((ground_count < GRASS_POINT_THR) or (dis2>0.4*0.4) or (count > TOTAL_POINT_THR)):
    # if(count > TOTAL_POINT_THR) or (max_point[0]!=0 and dis2>0.8*0.8 and (max_point[2]-min_point[2] < 0.25)): # 更改！
    # if(0):
        print("grass scene")
        for i in range(count):
            ...
            if ((point[2] > person_z1) and (point[2] < person_z2)):
                ...
                if(idx >= grass_grid_point_thr):  # 更改！
    ...
    # 如果不是草地直接分配到格子中
        ...
        for i in range(count):
            ...
            if(idx >= plane_grid_point_thr):  # 更改！
                    if(x_offset*x_offset + y_offset*y_offset < nearest_grid_x*nearest_grid_x + nearest_grid_y*nearest_grid_y):
                        nearest_grid_x = x_offset
                        nearest_grid_y = y_offset
...
# [ERROR] [1668393198.645179]: bad callback: <function lidar_callback at 0x7f455e85c1f0>
# Traceback (most recent call last):
#   File "/opt/ros/noetic/lib/python3/dist-packages/rospy/topics.py", line 750, in _invoke_callback
#     cb(msg)
#   File "SafeDet_multimode.py", line 221, in lidar_callback
#     point = grid_map[nearest_grid_x][nearest_grid_y][i]
# IndexError: list index out of range
        ...
        Output_PC[0][0] = nearest_x
        Output_PC[0][1] = nearest_y
        Output_PC[0][2] = -0.55
    ...
    if (Output_multiPC):
        ...
        for i in range(X_GRID_NUM):
            for j in range(Y_GRID_NUM):
                if (ground_count < GRASS_POINT_THR):
                    grid_point_thr = grass_grid_point_thr
                else:
                    grid_point_thr = plane_grid_point_thr
                if (grid_map_point_num[i][j] > grid_point_thr):
                    max_x = -1000
                    max_y = -1000
                    max_z = -1000
                    min_x = 1000
                    ...
                    Output_PC[obj_pc_count][0] = min_x
                    Output_PC[obj_pc_count][1] = min_y
                    Output_PC[obj_pc_count][2] = -0.55
                    obj_pc_count += 1
                    Output_PC[obj_pc_count][0] = min_x
                    Output_PC[obj_pc_count][1] = max_y
                    Output_PC[obj_pc_count][2] = -0.55
                    obj_pc_count += 1
                    Output_PC[obj_pc_count][0] = max_x
                    Output_PC[obj_pc_count][1] = min_y
                    Output_PC[obj_pc_count][2] = -0.55
                    obj_pc_count += 1
                    Output_PC[obj_pc_count][0] = max_x
    ...
    nearest_grid_points = np.array(Output_PC)
