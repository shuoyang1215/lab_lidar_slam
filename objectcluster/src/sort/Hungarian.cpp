...
#include "../../include/sort/Hungarian.h"
...
void HungarianAlgorithm::assignmentoptimal(int *assignment, double *cost, double *distMatrixIn, int nOfRows, int nOfColumns)
{
    ...
    /* initialization */
    *cost = 0;
    for (row = 0; row<nOfRows; row++)//trk分配初始化
        assignment[row] = -1;
    ...
    distMatrixEnd = distMatrix + nOfElements;//内存的结束指针
    ...
    /* preliminary steps */
    if (nOfRows <= nOfColumns)//检测出的框比上一帧预测的框要多，也就是这一帧有新的检测或者有的预测框到图像外面去了
    {
        minDim = nOfRows;

        for (row = 0; row<nOfRows; row++)//每行减去每行最小值
        {
            /* find the smallest element in the row */ //检测框去匹配跟踪框，在所有检测框里找一个与当前跟踪框距离最小的
        ...
        /* Steps 1 and 2a */ //给每个trk分配一个det
        for (row = 0; row<nOfRows; row++)
            for (col = 0; col<nOfColumns; col++)
                // DBL_EPSILON：https://blog.csdn.net/cutemypig/article/details/121891642
                // 即 distMatrix[row + nOfRows*col]=0
                if (fabs(distMatrix[row + nOfRows*col]) < DBL_EPSILON)
                    if (!coveredColumns[col])  // 该列有0,并且之前没有出现过0
                    {
    ...
    else /* if(nOfRows > nOfColumns) */ //每列减去每列的最小值
    {
        minDim = nOfColumns;
...
void HungarianAlgorithm::step2b(int *assignment, double *distMatrix, bool *starMatrix, bool *newStarMatrix, bool *primeMatrix, bool *coveredColumns, bool *coveredRows, int nOfRows, int nOfColumns, int minDim)
{
    ...
    if (nOfCoveredColumns == minDim)//匹配完了
    {
        /* algorithm finished */
...
void HungarianAlgorithm::step3(int *assignment, double *distMatrix, bool *starMatrix, bool *newStarMatrix, bool *primeMatrix, bool *coveredColumns, bool *coveredRows, int nOfRows, int nOfColumns, int minDim)
{
    ...
    while (zerosFound)
    {
        zerosFound = false;
        for (col = 0; col<nOfColumns; col++)
            if (!coveredColumns[col])//这一个det和track没匹配上
                for (row = 0; row<nOfRows; row++)
