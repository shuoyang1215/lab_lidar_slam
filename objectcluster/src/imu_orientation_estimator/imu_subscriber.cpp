...
//#include "imu_orientation_estimator/config.h"
...
//    imu_data.linear_acceleration.x = imu_msg_ptr->linear_acceleration.x*9.8;
//    imu_data.linear_acceleration.y = imu_msg_ptr->linear_acceleration.y*9.8;
//    imu_data.linear_acceleration.z = imu_msg_ptr->linear_acceleration.z*9.8;
//
...
//        LOG_IF(INFO, Config::use_glog) << std::fixed << "imu time: "<<imu_msg_ptr->header.stamp.toSec();
    new_imu_data_.push_back(imu_data);

    if(!is_start_sub){   // 如果还没有开始运行接受信息的节点，则清除imu数据
...
void IMUSubscriber::StartSubscribe() {
    is_start_sub = true;
}

void IMUSubscriber::StopSubscribe() {
    is_start_sub = false;
}
...
void IMUSubscriber::clear_state(){
    buff_mutex_.lock();
    new_imu_data_.clear();
    buff_mutex_.unlock();
}